...
- Mac Settings (SwiftUI) store `TYPESAFE_API_KEY` in the Keychain and non-secrets in UserDefaults. Share / Services read the server URL and provider only. The web app still reads the key from the server process environment, never from the browser or the ingest URL. Unsigned Mac binaries are published by `.github/workflows/mac-release.yml` (rolling `mac-latest`). Notarization is north-star.
