...
- Mac v1 hosts the same web UI in a WKWebView and starts a localhost-only Node server (static files + `/api/decide` + `/api/save` + `/api/export` + `/share`) on launch. Confirmed `http(s)` and `mailto:` URLs open in the system handler. Settings (SwiftUI, `⌘,`) store `TYPESAFE_API_KEY` in the Keychain and non-secrets in UserDefaults. The app injects the Keychain key into the bundled server environment. Services / `pastepilot://ingest` open the app window with `?text=` — not a browser. The web app still reads the key from the server process environment, never from the WKWebView or the ingest URL. Unsigned Mac binaries are published by `.github/workflows/mac-release.yml` (rolling `mac-latest`). Notarization is north-star.
