...
- Execution gate rechecks current-state rules immediately before any action. Confirm is required. After the gate, local adapters may run: **Open link / search / maps** (allowlisted `http`/`https` only), **Draft email** (`mailto:` draft, never send), **Save** (append to a local inbox), **Copy**, **Download** (`.ics` / `.json` / `.log` via `POST /api/export` or a browser download), and **Mac actions** (`POST /api/mac`: Notes, Reminders, Calendar `.ics` open, Finder, Safari/Chrome, Dictionary, Spotlight, Terminal-at-path, Shortcuts, `say`, share notify). Web/Linux fail open to labeled fallbacks. No calendar write, no pasted shell, no unrelated network write. The UI still shows at most three suggestions from a larger allowlist.
...
- Mac v1 hosts the same web UI in a WKWebView and starts a localhost-only Node server (static files + `/api/decide` + `/api/save` + `/api/export` + `/api/mac` + `/share`) on launch. Confirmed `http(s)`, `mailto:`, `dict:`, `notes:`, `shortcuts:`, and Reminders/Calendar schemes open in the system handler. Settings (SwiftUI, `⌘,`) store `TYPESAFE_API_KEY` in the Keychain and non-secrets (including preferred browser and Shortcut name) in UserDefaults. The app injects the Keychain key and those prefs into the bundled server environment — never onto a URL. Services / `pastepilot://ingest` open the app window with `?text=` — not a browser. The web app still reads the key from the server process environment, never from the WKWebView or the ingest URL. Unsigned Mac binaries are published by `.github/workflows/mac-release.yml` (rolling `mac-latest`). Notarization is north-star.
