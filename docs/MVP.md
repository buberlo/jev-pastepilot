...
- **Mac Confirm tools** — after Confirm, the Mac app runs side-effects in **Swift** (NSWorkspace / NSAppleScript / NSSpeechSynthesizer / share sheet): Notes, Reminders, Calendar `.ics` open, Finder, enclosing folder, Safari/Chrome, preferred editor, Preview, Desktop/Downloads saves, special-folder reveal, Apple Maps, `tel:`/`sms:`, vCard, Dictionary, Spotlight copy, Terminal at a path (never a pasted shell command), Settings-named Shortcut, speak. The bundled Node server does not spawn `osascript`. Web/Linux use labeled fallbacks. Deterministic steals rank Finder, Open link, Draft email, Call, Maps, Format JSON, Save code, Look up, and Calendar for those strong signals even when live Jev would pick something else. **Screen paste** is a local injection/substance summary on every OS.
