...
- **Mac Confirm tools** — after Confirm, the Mac app runs side-effects in **Swift** (NSWorkspace / NSAppleScript / NSSpeechSynthesizer): Notes, Reminders, Calendar `.ics` open, Finder, Safari/Chrome, Dictionary, Spotlight copy, Terminal at a path (never a pasted shell command), Settings-named Shortcut, speak, share notify. The bundled Node server does not spawn `osascript`. Web/Linux use labeled fallbacks. A deterministic path steal ranks **Open in Finder** / **Open in Terminal** for `/Users/…` and `~/…` even when live Jev would pick something else. **Screen paste** is a local injection/substance summary on every OS.
